 ...
 * @author Admin
 */
public class OyenteEliminarPublicacion {
    private String HOST= "127.0.0.1";
    private int PORT= 5000;
    private Socket socket;
    private BufferedReader bufferedReader;
    private BufferedWriter bufferedWriter;
    private IObservadorEliminarPublicacion observador;
    
    public OyenteEliminarPublicacion(IObservadorEliminarPublicacion observador){
        try{
    ...
    private void registrarObservador(){
        Solicitud solicitud= new Solicitud(Operacion.suscribir_observador_eliminarPublicacion);
        String mensaje= Deserealizador.getInstancia().serializarSolicitud(solicitud);
        this.enviarMensaje(mensaje);
    }
    
    public void eliminarObservador(){
        Solicitud solicitud= new Solicitud(Operacion.desuscribir_observador_eliminarPublicacion);
        String mensaje= Deserealizador.getInstancia().serializarSolicitud(solicitud);
        this.enviarMensaje(mensaje);
    }
            ...
            public void run(){
                ...
                while(socket.isConnected()){
                    try{
                        ...
                        observador.notificarEliminacionPublicacion(Deserealizador.getInstancia().deserealizarPublicacion(mensajeOyenteBroker));
                    } catch (IOException e){
                        cerrarTodo(socket, bufferedReader, bufferedWriter);
                    }
                }
